                       ...
                       int windowBits, int memLevel, int strategy, const char *version, int stream_size)
{
    ...
    {
        mock_ret = _mock_zlib->deflateInit2_(file, line, func, strm, level, method, windowBits, memLevel, strategy,
                                           version, stream_size);
    }
    else
    {
        mock_ret = delegate_real_deflateInit2_(file, line, func, strm, level, method, windowBits, memLevel, strategy,
                                             version, stream_size);
    }
