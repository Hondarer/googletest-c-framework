...
bool is_console_handle(HANDLE handle)
{
    ...
    return handle != nullptr && handle != INVALID_HANDLE_VALUE && GetFileType(handle) == FILE_TYPE_CHAR &&
...
void reader_thread_proc(StreamState *state)
{
    ...
    {
        DWORD nread = 0;
        const BOOL ok = ReadFile(state->pipe_read, buf + pending_len, kReadBufferSize - static_cast<DWORD>(pending_len),
                                 &nread, nullptr);
        if (!ok || nread == 0)
        {
            break;
        ...
        if (complete > 0)
        {
            const int wide_len = MultiByteToWideChar(CP_UTF8, 0, buf, complete, wide_buf, kReadBufferSize);
            if (wide_len > 0)
            {
                DWORD nwritten = 0;
                if (!WriteConsoleW(state->orig_handle, wide_buf, static_cast<DWORD>(wide_len), &nwritten, nullptr))
                {
                    WriteFile(state->orig_handle, buf, static_cast<DWORD>(complete), &nwritten, nullptr);
                }
            }
            else
            {
    ...
    {
        DWORD nwritten = 0;
        WriteFile(state->orig_handle, buf, static_cast<DWORD>(pending_len), &nwritten, nullptr);
    }
}
...
{
    return is_console_handle(GetStdHandle(STD_OUTPUT_HANDLE)) || is_console_handle(GetStdHandle(STD_ERROR_HANDLE));
}
