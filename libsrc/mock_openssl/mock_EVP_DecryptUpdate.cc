...
int mock_EVP_DecryptUpdate(const char *file, const int line, const char *func, EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl, const unsigned char *in, int inl)
{
    ...
    {
        mock_ret = _mock_openssl->EVP_DecryptUpdate(file, line, func, ctx, out, outl, in, inl);
    }
    else
    {
        mock_ret = delegate_real_EVP_DecryptUpdate(file, line, func, ctx, out, outl, in, inl);
    }
