...
GoogleTest は、テスト実行結果の視認性を高めるため、ターミナル出力に ANSI カラー コードを使用します。このドキュメントでは、出力行の見出し部 (角括弧で囲まれた部分) に適用される色を説明します。
...
./test_binary --gtest_color=auto  # 自動判定 (既定)
```
...
`--gtest_color=auto` (既定) の場合、以下の条件をすべて満たすときに色が有効になります。
...
Windows 環境などでエスケープ シーケンスなしの出力に着色するフィルター スクリプトについて説明します。

```bash
test.exe | python add_gtest_color.py
```

このフィルターは、見出し部 (角括弧で囲まれた部分) のみを着色し、本文はそのまま出力します。
...
2. **将来の GoogleTest バージョン**: 新しい見出しが追加された場合は、フィルターの更新が必要となる可能性

3. **サード パーティー ツールの出力**: GoogleTest 互換の書式でも、微妙に異なるスペーシングを使用している場合
