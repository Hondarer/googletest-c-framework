# VSBT でのカバレッジ ツール選択ガイド

## VSBT でのカバレッジ ツールの選択肢

VSBT (Visual Studio Build Tools) 環境で、Enterprise ツールを使わない場合の選択肢を整理します。

### OpenCppCoverage (推奨)

Windows 向けのオープンソース カバレッジ ツールです。gcov に最も近い使い勝手で、MSVC でビルドしたバイナリに対応しています。
...
商用ツールですが、MSVC に対応しており、ソース コードのインストルメンテーション (コードに計測用の処理を埋め込むこと) を行ってカバレッジを取得します。ライセンス費用が発生します。
...
OpenCppCoverage は名前に "Cpp" と入っていますが、C 言語と C++ の両方に対応しています。MSVC でコンパイルされた C 言語のプログラムでもカバレッジを取得できます。
...
OpenCppCoverage は PDB (Program Database) ファイルを使ってソース コードとバイナリの対応関係を取得します。この仕組みは C 言語でも C++ でも同じように機能します。

## OpenCppCoverage を利用して gcov 形式のシンプルなテキストを生成する方法

### 概要

OpenCppCoverage は gcov 形式のテキスト出力に直接対応していませんが、Cobertura XML 形式で出力し、それを変換することで gcov 互換のテキスト ファイルを生成できます。
