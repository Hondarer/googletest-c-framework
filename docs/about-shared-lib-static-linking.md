...
オプション 1 (推奨アプローチ) を自動的に実現します。`LIBS` に指定された静的ライブラリを共有ライブラリに組み込むことで、依存関係の管理を簡素化します。
...
4. システム ライブラリ パス (`/usr/lib`, `/usr/local/lib`, `/lib`, etc.)
...
### 例 1: LIBSDIR を使用 / Example 1: Using LIBSDIR
...
### 例 3: 直接 .a を指定 / Example 3: Direct .a specification
...
### 例 4: 混在パターン / Example 4: Mixed pattern
...
- `-Wl,-rpath,/opt/lib` → リンク オプションとしてそのまま渡される
...
x86_64 Linux 用のシステム ライブラリ パス (`/usr/lib/x86_64-linux-gnu` など) がハード コードされています。他のアーキテクチャーでは、`makelibsrc.mk` の `ALL_LIB_DIRS` に適切なパスを追加する必要があります。
