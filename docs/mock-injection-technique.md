...
**必須:** `-I include_override` は最も優先度の高いインクルード パスに設定してください。
...
1. **ヘッダー インターセプション**: インクルード パスの優先順位を利用
...
このテクニックにより、既存の C コードを一切変更することなく、標準ライブラリ関数の動作を完全に制御できます。これは、レガシ コードのテスタビリティを劇的に向上させる強力な手法です。
