...
- 実体 (メモリの確保) は **1 か所のソース ファイル** にだけ必要である。
...
- コンパイルは通るが **リンク時に未定義シンボル エラー** になる。
...
しかし、**テストやモック ライブラリの構成時に例外的に許される手法がある**。
...
- `#pragma weak` は **GCC などの一部コンパイラに依存** する。
- 本番コードでは使用を避け、**主にユニット テストのための手法** として理解する。
...
| 実体 (定義)              | メモリを確保する唯一の場所                 |
| ヘッダーでの実体定義     | 複数ファイルで include されると危険        |
| C++ 相互運用             | `extern "C"` で C の名前修飾を防止         |
| `static`                 | ファイル内限定のシンボル                   |
