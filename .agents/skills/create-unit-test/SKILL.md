...
- テスト本体の `{` の次の非空行が `// Arrange` であり、ローカル変数やモック オブジェクトの宣言が先行していないこと
- `// Arrange`、`// Pre-Assert`、`// Act`、`// Assert` が存在し、明示的な後処理がある場合だけ `// Cleanup` が存在すること
- 被テスト関数の呼び出しが `EXPECT_*` / `ASSERT_*` (`EXPECT_CALL` を除く) の引数に埋め込まれていないこと
- Assert フェーズの各 `ASSERT_*` マクロに、検証内容と一対一に対応する `[確認_正常系]` または `[確認_異常系]` のコメントが付与されていること
- 戻り値を確認するコメントに、Act で呼び出した関数名が記載されていること
- Arrange フェーズで確保した資源やオブジェクトの ASSERT/EXPECT に `[状態確認]` が付与されていること (`[確認_*]` と混同していないこと)
...
- `make test` 後に `results.log` のサマリーを確認し、次の状態になっていないこと
    ...
    - 同一の説明文が複数回出力されている
    - `[状態確認]` の行が `### 確認内容` の先頭に `- (説明文)` の形式で出力されていない、または確認件数に計上されている
